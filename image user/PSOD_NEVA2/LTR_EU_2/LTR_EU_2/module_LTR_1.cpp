...
static  std::string  CREAT_SN = reader.Get("MODULE_1", "CREAT_SN", "UNKNOWN"); // Серийный номер крейта 
static  const int SLOT_NUMBER = reader.GetInteger("MODULE_1", "SLOT_NUMBER", -1); // Номер слота в крейте
static  const int ACQ_BLOCKS_QNT = reader.GetInteger("MODULE_1", "ACQ_BLOCKS_QNT", -1); // Колличество блоков в буфере сбора блока
static  const int TOTAL_BLOCKS_QNT = reader.GetInteger("MODULE_1", "TOTAL_BLOCKS_QNT", -1); // не меняем 
static  const int POINTS_PER_CHANNEL = reader.GetInteger("MODULE_1", "POINTS_PER_CHANNEL", -1); // 
static  const int CHANNELS_QNT = reader.GetInteger("MODULE_1", "CHANNELS_QNT", -1);; // Количество каналов 
static  double SLOT_RATE = reader.GetInteger("MODULE_1", "SLOT_RATE", -1); // Частота считывания блоков Гц
static  double limit_DPK = reader.GetReal("MODULE_1", "Limit_DPK", -1); // Предел срабатывания ДПК mV
static  volatile int limit_time = reader.GetInteger("MODULE_1", "Limit_time", -1); //600сек
static  volatile int Time_stop = reader.GetInteger("MODULE_1", "Time_stop", -1); // Время блока
static  volatile int Chanel_DPK = reader.GetInteger("MODULE_1", "Chanel_DPK", -1); // Канал ДПК
static  volatile int Filter_ON_OFF = reader.GetInteger("MODULE_1", "Filter_ON_OFF", -1); // ВКЛ ВЫКЛ фильтр
static  std::string  file_way = reader.Get("MODULE_1", "File_way", "UNKNOWN");  // Путь к файлам хранения данных
static  std::string FileNamedb = "";
...
static DWORD WINAPI AcquireThread(LPVOID param) // Функция сбора данных с крейта 
{
	...
	while (RunFlag) // Пока запущен сбор данных...
	{
		...
		START_potok_1 = 1;
		while((START_potok_1 == 0) and (START_potok_2 == 0)){}
...
DWORD WINAPI POTOK_1(LPVOID param) // Функция записи данных в файл
{
	static  TLTR ltr;
	int ltr_result;
	...
	hltr212.AcqMode = reader.GetInteger("MODULE_1", "AcqMode", -1); // 4-канальный режим высокой точности - "1" 4-канальный режим средней точности - "0"
	hltr212.UseClb = reader.GetInteger("MODULE_1", "UseClb", -1); //  Не используем пользовательские калибровочные коэфф. "1" при AcqMode = 0 ,"0" при AcqMode = 1 
	hltr212.UseFabricClb = reader.GetInteger("MODULE_1", "UseFabricClb", -1); // Используем заводские калибр. коэфф. "1" при AcqMode = 1 ,"0" при AcqMode = 0
	hltr212.LChQnt = reader.GetInteger("MODULE_1", "LChQnt", -1); // Количество логических каналов - 4


	for (i = 0; i < hltr212.LChQnt; i++)
		hltr212.LChTbl[i] = LTR212_CreateLChannel(i + 1, reader.GetInteger("MODULE_1", "LChTbl", -1));
	...
	else
	{
		...
		hltr212.filter.Decimation = reader.GetInteger("MODULE_1", "Decimation", -1);
		hltr212.filter.TAP = reader.GetInteger("MODULE_1", "TAP", -1);
		strcpy(hltr212.filter.IIR_Name, reader.Get("MODULE_1", "Filter_IIR", "UNKNOWN").c_str());
		strcpy(hltr212.filter.FIR_Name, reader.Get("MODULE_1", "Filter_FIR", "UNKNOWN").c_str());
	}
	
	hltr212.REF = reader.GetInteger("MODULE_1", "REF", -1); // Опорное напряжение 5 В.
	hltr212.AC = reader.GetInteger("MODULE_1", "AC", -1);; // Постоянное опорное напряжение
	...
	strcpy(MsgString, "Поток сбора данных запущен ------ MODULE_1!\n");
	CharToOem((wchar_t*)MsgString, MsgString);
	setlocale(LC_ALL, "Russian");
	printf("%s\n", MsgString);
	int flag = 0;
	...
	printf("FLAGS START POTOK MODULE 1\n");
		...
		if (BlockReady[BlockCntr]) // Если очередной блок считан
		{
			if (START_write_1 == 0)
			{
				...
				if (flag_ok_dpk_block == 1) {
					...
					FileNamedb = file_way + NAME_SYSTEM + '_' + FileName2 + "_" + FileName5 + "_";
	...
	strcpy(MsgString, "Поток сбора данных удален ------ MODULE_1\n");
	CharToOem((wchar_t*)MsgString, MsgString);
	setlocale(LC_ALL, "Russian");
	printf("%s\n", MsgString);
	fclose(DataFile);
	...
	flag_delete_file = 0;
	strcpy(MsgString, "Файл для записи данных закрыт ------ MODULE_1\n");
	CharToOem((wchar_t*)MsgString, MsgString);
	setlocale(LC_ALL, "Russian");
	printf("%s\n", MsgString);
	RunFlag = 1;
